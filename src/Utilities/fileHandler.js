...
export async function readRSFiles(files) {
    ...
    for (let file of files) {
        ...
        if (extension === "dcm" && type === "RS") {
            let newVal = await readRS(file)
            res = [...res, ...newVal]
        }
    }
    return res;
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = () => {
            ...
            try {
                ...
                resolve(readStructs(dataSet));
            }
            catch (ex) {
                reject(`Error parsing byte stream ${ex}`);
            }
